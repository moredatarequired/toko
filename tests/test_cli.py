...
def test_a_warm_cache_counts_every_file_when_descriptors_are_scarce(tmp_path):
    ...
    counted = [int(count) for _, count, _approximate in cold_rows[1:]]
    warm_rows = _csv_rows(warm.stdout)[1:]
    assert sum(int(count) for _, count, _approximate in warm_rows) == sum(counted)
