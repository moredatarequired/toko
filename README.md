...
- **Git's ignore files need a git repository.** In a directory that is not inside one, `.gitignore` is not read at all, and a file you excluded with it *will* be counted — and for the API-backed models (Anthropic, Google, xAI) its contents are sent to that provider. This is what `rg --files` does; `--no-require-git` is how you switch it off there, and toko has no equivalent. **Use `.ignore` or `.rgignore` instead**: they take the same patterns, they are honoured all the way to the filesystem root whether or not a repository exists, and they outrank `.gitignore` where both apply. Renaming a stray `.gitignore` to `.ignore` restores the old behaviour exactly.
