...
`google/` and `huggingface/` are left out of that set, because both are Hub organisations that own real repos, so `google/gemma-3-1b-it` stays that repo rather than becoming Google's `gemma-3-1b-it`. That exemption wins over the spelling `--list-models` prints, and the two rules collide on the five retired Gemini models: `xai/grok-3` is refused as retired, but `google/gemini-2.0-flash-001` is not — it is read as a Hub repo id, and since no such repo answers, the run fails with `not found on HuggingFace` rather than with the retirement error. The same holds for `google/gemini-2.0-flash-lite-001`, `google/gemini-2.0-flash-preview-image-generation`, `google/gemini-3-pro-preview`, and `google/gemini-3.1-flash-lite-preview`. Drop the `google/` prefix to get the retirement refusal.
